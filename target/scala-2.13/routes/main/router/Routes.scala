...
class Routes(
  ...
  def documentation = List(
    ("""GET""", this.prefix + (if(this.prefix.endsWith("/")) "" else "/") + """api/v1/organizations/""" + "$" + """orgName<[^/]+>/contributors""", """controllers.HomeController.index(orgName:String)"""),
  ...
  def routes: PartialFunction[RequestHeader, Handler] = {
    ...
    case controllers_HomeController_index0_route(params@_) =>
      call(params.fromPath[String]("orgName", None)) { (orgName) =>
        controllers_HomeController_index0_invoker.call(HomeController_1.index(orgName))
      }
